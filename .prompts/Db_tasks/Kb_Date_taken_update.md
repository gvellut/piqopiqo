...
New folder:
when reading a **new** folder with a large number of photos and state of the app has sort order as Time taken: blink of the preview images. Seems to be replaced by another item every few ms :  shows something else (thumbnail not loaded so seems blank). The preview thumbnail (or more exactly the ref to the real image underneath) seems to change. the labels below the image (file name + date) seem to change as well.
...
=> If new : and date taken sort (if sort by filename or file name by folder; should be fine from the start since obtained from the start ie do the sort before displaying anything) display by scanned order then sort only when exiftool of all the files have been extracted after a load folder : so done at once. If selected keep in visible in new order
	if Handle some errors for some file : must still sort after
...
=> if new images added or deleted while running : resort when event received (no waiting in that case)
=> if loading and new photos scanned with no thumbnail or entry in Metadata DB : also use the runtime setting so no blink
