- When using the filter : for example clear filter, or filter on a label and maybe (but not sure) setting the folder : I see some delay after I click and the item on which I click changing, for ex, the checkbox becoming checked. Make sure the feedback to the click is instatenous and that whatever action is being performed either happens in the background or at least after the feedback.
