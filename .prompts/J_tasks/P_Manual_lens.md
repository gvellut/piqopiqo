...
Confim dialog : choose lens, info of which information will be set + number of images : ok cancel
Set the lens info for all the selected in grid. Error if none is selected.

In metadata DB (hidden) => create those fields in DB for folder. No need to precreate them : only when the Manuel lens tool is used.
then in save Exif + Flickr upload  : set only if those exists (ie non empty) then set (like it is now I think)

In User Settings : List to add lens info : list the keys. add, delete, edit buttons.
Add + edit : form with the values to be filled by the user. Use placeholders (use Samyang below)
