...
def _resolve_default_cache_base_dir() -> str:
    return str(get_cache_base_dir_candidate())


def _resolve_default_exiftool_path() -> str | None:
...
_MANDATORY_SETTING_SPECS: tuple[MandatorySettingSpec, ...] = (
    MandatorySettingSpec(
        ...
        default_resolver=_resolve_default_cache_base_dir,
    ),
    MandatorySettingSpec(
        ...
        default_resolver=_resolve_default_exiftool_path,
    ),
)
...
def _resolve_support_dir_path(*, create: bool) -> Path:
    if sys.platform == "darwin":
...
def get_support_dir_candidate() -> Path:
    """Get support directory path without creating it."""
    return _resolve_support_dir_path(create=False)


def get_support_dir() -> Path:
    """Get the platform-specific application support directory."""
    return _resolve_support_dir_path(create=True)


def get_cache_base_dir_candidate() -> Path:
    """Get the default cache base path without creating it."""
    return get_support_dir_candidate() / "cache"


def get_cache_base_dir() -> Path:
    """Get the default fallback base directory for cache data."""
    cache_dir = get_cache_base_dir_candidate()
