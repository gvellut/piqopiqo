...
keep zoom when navigating image
reproduce the same zoom : mouse, click, navigate, max zoom as Bridge
=>
...
like single wheel : zoom on clicked pixel to actual pixel size : hardware pixel size
other single click : go back
verify pixel of image is physical pixels and not logical


when unzoom : keep under the mouse even if corners 
Check what Brdige does

filter : folder for now
order : by some exid attribute (DTO)

number of columns : settable in GUI
