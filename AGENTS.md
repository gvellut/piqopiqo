...
- Grid memory policy: embedded previews are kept in memory once loaded; HQ thumbnails are only kept for the visible range plus `Config.GRID_THUMB_BUFFER_ROWS` and are always preferred for display in that buffered range. HQ eviction only happens outside that buffered range, and selection state does not affect which thumbnail quality is shown.
